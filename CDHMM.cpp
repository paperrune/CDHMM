...
void Continuous_Density_Hidden_Markov_Model::Search_State_Sequence(int event_index, int state_index, int state_sequence[], vector<int*> gamma){
...
bool Continuous_Density_Hidden_Markov_Model::Access_State(int previous_state_index, int state_index){
	int i = previous_state_index;
	int j = state_index;

	return (type_model == "ergodic") || (j - i == 0) || (j - i == 1);
}

double Continuous_Density_Hidden_Markov_Model::Backward_Algorithm(int length_event, vector<int> state, double **beta, double likelihood[]){
	...
	for (int t = length_event - 1; t >= 0; t--){
			...
			for (int i = 0; i < state.size(); i++){
				...
				for (int j = 0; j < state.size(); j++){
					...
					if (state_connection[k].find(l) != state_connection[k].end() && Access_State(i, j)){
						sum += transition_probability[k].find(l)->second * likelihood[(t + 1) * number_states + l] * beta[t + 1][j];
					}
				}
				scale += (beta[t][i] = sum);
			}
		...
		if (!_finite(log(scale = 1 / scale)) || _isnan(log(scale))){
			cerr << "[Backward Algorithm], scale[" << t << "]: " << scale << endl;
		}
		else {
...
double Continuous_Density_Hidden_Markov_Model::Forward_Algorithm(int length_event, vector<int> state, double **alpha, double likelihood[]){
	double log_likelihood = 0;
	...
	for (int t = 0; t < length_event; t++){
		...
		if (t == 0){
			for (int i = 0; i < state.size(); i++) {
				...
				scale += (alpha[t][i] = (type_model == "ergodic" || i == 0) * initial_probability[j] * likelihood[t * number_states + j]);
			}
		}
		else{
			for (int i = 0; i < state.size(); i++){
				...
				scale += (alpha[t][i] = sum * likelihood[t * number_states + state[i]]);
			}
		}

		// scale
		if (!_finite(log(scale = 1 / scale)) || _isnan(log(scale))){
			cerr << "[Forward Algorithm], scale[" << t << "]: " << scale << endl;
		}
		else {
			log_likelihood += log(scale);
		}
...
Continuous_Density_Hidden_Markov_Model::Continuous_Density_Hidden_Markov_Model(string path) {
	ifstream file(path);
	...
	if (file.is_open()) {
		...
		state_connection = new set<int>[number_states];
		...
		cerr << "[Continuous_Density_Hidden_Markov_Model], " + path + " not found" << endl;
	}
}
Continuous_Density_Hidden_Markov_Model::Continuous_Density_Hidden_Markov_Model(set<int> state_connection[], string type_model, vector<string> state_label, int dimension_event, int number_states, string type_covariance, int number_gaussian_components){
	...
	this->state_connection	= new set<int>[number_states];
...
Continuous_Density_Hidden_Markov_Model::~Continuous_Density_Hidden_Markov_Model(){
	if (GMM) {
		for (int i = 0; i < number_states; i++) {
			delete GMM[i];
		}
	...
	if(initial_probability)		delete[] initial_probability;
	if(state_connection)		delete[] state_connection;
	if(transition_probability)	delete[] transition_probability;
	if(valid_transition)		delete[] valid_transition;
}

void Continuous_Density_Hidden_Markov_Model::Initialize(int number_events, int length_event[], double **_event){
	for (int i = 0; i < number_states; i++){
	...
	if (GMM) {
		...
		for (int i = 0, index = 0; i < number_events; i++){
			for (int j = 0; j < length_event[i]; j++){
				data[index++] = &_event[i][j * dimension_event];
			}
		}
...
void Continuous_Density_Hidden_Markov_Model::Save_Model(string path) {
	ofstream file(path);
...
double Continuous_Density_Hidden_Markov_Model::Baum_Welch_Algorithm(int number_events, int length_event[], vector<int> state[], double minimum_variance, double **_event){
	double log_likelihood = 0;
	...
	for (int h = 0; h < number_events; h++){
		int number_states = static_cast<int>(state[h].size());

		double *likelihood = new double[length_event[h] * this->number_states];
		...
		for (int t = 0; t < length_event[h]; t++){
			for (int i = 0; i < number_states; i++) {
				...
				for (int j = 0; j < number_gaussian_components; j++) {
					gaussian_distribution[t][i][j] = GMM[k]->Gaussian_Distribution(&_event[h][t * dimension_event], j);
				}
				likelihood[t * this->number_states + k] = GMM[k]->Calculate_Likelihood(&_event[h][t * dimension_event], gaussian_distribution[t][i]);
			}
		}
				...
				for (int i = 0; i < number_states; i++){
					for (int j = 0; j < number_states; j++){
						...
						if (Access_State(i, j) && state_connection[k].find(l) != state_connection[k].end()) {
							sum_delta += (delta[i][j] = alpha[t][i] * transition_probability[k].find(l)->second * likelihood[(t + 1) * this->number_states + l] * beta[t + 1][j]);
						}
						else {
							delta[i][j] = 0;
						}
		...
		for (int j = 0; j < number_gaussian_components; j++) {
			for (int k = 0; k < dimension_event; k++) {
				for (int m = 0; m < number_states; m++) {
					...
					for (int t = 0; t < length_event[h]; t++) {
						sum[0] += theta[m][j][t] * _event[h][t * dimension_event + k];
						sum[1] += theta[m][j][t];
					}
					#pragma omp atomic
			...
			for (int k = 0; k < dimension_event; k++) {
				if (type_covariance == "diagonal") {
					for (int n = 0; n < number_states; n++) {
						...
						for (int t = 0; t < length_event[h]; t++) {
							sum[0] += theta[n][j][t] * (_event[h][t * dimension_event + k] - GMM[state[h][n]]->mean[j][k]) * (_event[h][t * dimension_event + k] - GMM[state[h][n]]->mean[j][k]);
							sum[1] += theta[n][j][t];
						}
						#pragma omp atomic
					...
					for (int l = 0; l < dimension_event; l++) {
						for (int n = 0; n < number_states; n++) {
							...
							for (int t = 0; t < length_event[h]; t++) {
								sum[0] += theta[n][j][t] * (_event[h][t * dimension_event + k] - GMM[state[h][n]]->mean[j][k]) * (_event[h][t * dimension_event + l] - GMM[state[h][n]]->mean[j][l]);
								sum[1] += theta[n][j][t];
							}
							#pragma omp atomic
	...
	return log_likelihood / number_events;
}
double Continuous_Density_Hidden_Markov_Model::Evaluation(int length_event, double **_event){
	double log_likelihood = 0;
...
double Continuous_Density_Hidden_Markov_Model::Viterbi_Algorithm(string *optimal_label_sequence, int **optimal_state_sequence, int length_event, double *_event){
	int *state_sequence = new int[length_event];

	double log_likelihood = 0;

	double *_likelihood = new double[length_event * number_states];

	vector<int*>	gamma;
	vector<double*> delta;
	vector<string*> label;

	for (int t = 0; t < length_event * number_states; t++) {
		_likelihood[t] = GMM[t % number_states]->Calculate_Likelihood(&_event[t / number_states * dimension_event]);
	}
	for (int t = 0; t < length_event; t++){
		double scale;
		double sum = 0;
		...
		double *likelihood = &_likelihood[t * number_states];
		...
		for (int i = 0; i < number_states; i++){
			...
			if (t == 0){
				...
				if (state_label[i] != state_label[argmax] /*|| i % 3 == 0 && argmax % 3 == 2*/){
					label[t][i] = state_label[i];
				}
			}
			sum_delta += delta[t][i];
...
double Continuous_Density_Hidden_Markov_Model::Baum_Welch_Algorithm(int number_events, int length_event[], vector<int> state[], double minimum_variance, double **_event, Neural_Networks *NN) {
	double log_likelihood = 0;
...
#pragma omp critical
		{
			NN->Test(_event[h], likelihood, length_event[h]);
		}
...
double Continuous_Density_Hidden_Markov_Model::Viterbi_Algorithm(string *optimal_label_sequence, int **optimal_state_sequence, int length_event, double *_event, Neural_Networks *NN) {
	int *state_sequence = new int[length_event];
	...
	NN->Test(_event, likelihood, length_event);
