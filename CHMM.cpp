			...
			for (int j = 0; j < this->number_states; j++){
				...
				for (int t = 0; t < length_event[h] - 1; t++){
					...
					for (int l = 0; l < number_states[h]; l++){
						delta[l] = new double[number_states[h]];
					}
					...
					for (int l = 0; l < number_states[h]; l++){
						for (int m = 0; m < number_states[h]; m++){
							...
							sum_delta += (delta[l][m] = (state_connection[n][o] && Access_State(l, m)) * alpha[t][l] * transition_probability[n][o] * likelihood[t + 1][m] * beta[t + 1][m]);
						}
					}
